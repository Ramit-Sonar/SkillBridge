...
export function ProjectOverview({
  ...
  const handleEmojiSelect = (emoji: string) => {
    setMessageDraft((currentDraft) => `${currentDraft}${emoji}`);
  };
  ...
  return (
    <div className="grid lg:grid-cols-3 gap-4 items-start">
      ...
      >
        ...
        <div className="relative flex items-center gap-2 bg-slate-50 rounded-xl border border-black/[0.06] px-3 py-2 focus-within:border-blue-200 focus-within:ring-4 focus-within:ring-blue-50 transition-all">
          {showEmojiPicker && (
            <div className="absolute bottom-12 left-10 z-10 grid w-44 grid-cols-4 gap-1 rounded-xl border border-slate-200 bg-white p-2 shadow-lg">
              {MESSAGE_EMOJIS.map((emoji) => (
                <button
                  ...
                  disabled={messageSending}
                  className="flex h-8 w-8 items-center justify-center rounded-lg text-base hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
